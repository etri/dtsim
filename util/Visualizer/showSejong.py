...
if opt_road_enable == 1:
    ...
    for shape in sf.shapes():
        ap = plt.Polygon(shape.points, closed=False, fill=False, alpha=0.5, linewidth=0.5, edgecolor="k")
        ax.add_patch(ap)
...
if opt_building_enable == 1 :
    sf_building = shapefile.Reader(opt_shape_path + "/NODE")
    for shape in sf_building.shapes():
        xval = float(shape.points[0][0])
        yval = float(shape.points[0][1])
        X.append(xval)
        Y.append(yval)

############### START ###############

plt.scatter(X, Y, s=4, c='black', marker='D')
